...
export default function CallbackForm() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await emailjs.send("service_2j77bum", "template_8m4nd0b", {
        name: formState.name,
  ...
  return (
    <>
        ...
        >
          ...
          <div className={styles.checkboxWrapper}>
            <input
              ...
              onChange={(e) =>
                setFormState({ ...formState, privacyPolicyConfirmation: e.target.checked })
              }
            />
            <label htmlFor="privacyPolicyConfirmation">
              ...
              <a
                ...
                target="_blank"
                aria-label="Политикой обработки данных"
              >
      ...
      {isSuccess === true && (
        <div className={styles.sendSuccess}>Ваше сообщение успешно отправлено</div>
      )}

      {isSuccess === false && (
        <div className={styles.sendFailed}>Сообщение не отправлено. Повторите попытку позже</div>
      )}
    </>
  );
}
